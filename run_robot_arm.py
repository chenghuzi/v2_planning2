...
def train_model(
    ...
    with torch.no_grad():
        ...
        # init_pos = torch.from_numpy(observation["observation"][0:3]).to(DEVICE)
        # N_g = 20
        # ultimate_goal_pos_all = torch.rand((N_g, 3)).to(DEVICE)
        # goal_min = torch.Tensor([-0.15, -0.15, 0]).to(DEVICE)
        # goal_max = torch.Tensor([0.15, 0.15, 0.3]).to(DEVICE)
        # goal_range = goal_max - goal_min
        # ultimate_goal_pos_all = goal_min + goal_range * ultimate_goal_pos_all

        # goal_diff = ultimate_goal_pos_all - init_pos
        # g_combined = []
        # for t in torch.linspace(0, 1, 10):
        #     tmp_goal = init_pos + t * goal_diff
        #     g_combined.append(tmp_goal)

        # g_combined = torch.concat(g_combined)
        # s_combined = torch.from_numpy(observation["observation"]).repeat(len(g_combined), 1).to(DEVICE)
        # dis_all = torch.norm(g_combined - init_pos, dim=1)
